...
Рассмотрим задачу выбора структуры для однослойной нейросети в задаче классификации:
\[
    f(\mathbf{x}) = \text{softmax}(\mathbf{W}^\text{T}\mathbf{f}_2(\mathbf{x})),
\]
\[
...
где $\mathbf{W}_1, \dots, \mathbf{W}_{2K}$ --- матрицы одинаковой размерности с различным количеством нулевых строк, $\{\mathbf{g}^{i}_{0,1},\mathbf{g}^{i}_{1,2}\}_{i=1}^K$ --- базовые функции для скрытого слоя нейросети.~\\~\\

\textbf{Структура модели} $\boldsymbol{\Gamma} = [\boldsymbol{\gamma}_{0,1}, \boldsymbol{\gamma}_{1,2}]$ определяется вершинами двух булевых $K$-мерных кубов.
