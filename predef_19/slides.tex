...
\[                                                                                                                                              
        Q = \text{log}p(\mathbf{h}|\mathbf{X}, \mathbf{y})= \textcolor{OliveGreen}{\text{log}p(\mathbf{h})} +  \text{log}\int\limits_{\mathbf{W}} \textcolor{red}{p(\mathbf{y}|\mathbf{X},\mathbf{W})} \textcolor{blue}{p(\mathbf{W}| \mathbf{h})} d\mathbf{W},                         
\]       
...
TODO: непонятно что делать с prior на гиперпараметры
...
\footnotesize
%\begin{block}{Определение}
Задачу выбора модели $\mathbf{h}^{*}, \boldsymbol{\theta}^{*}$ назовем обобщаюшей на множестве $U_{\theta}\times U_{h} \times U_{\beta} \subset \mathbb{R}^{u} \times \mathbb{H} \times \mathbb{B}$, если выполнены условия:
\begin{enumerate}
\item Для каждого $\mathbf{h} \in U_h$ и каждого $\boldsymbol{\beta} \in U_{\beta}$ решение $\boldsymbol{\theta}^{*}$ определено однозначно.
\item \textit{Условие максимизации правдоподобия выборки:} существует $\boldsymbol{\beta} \in U_{\beta}$ и  $K_1 \in \mathbb{R}_{+}$, такие что для любых векторов гиперпараметров $\mathbf{h}_1, \mathbf{h}_2 \in  U_{h}, Q(\mathbf{h}_1) - Q(\mathbf{h}_2) > K:$ соответствующие матожидания правдоподбия выборок упорядочены как $\mathsf{E}_q \text{log}~p(\mathbf{y}|\boldsymbol{\theta}_1, c_{\text{temp}})>\text{log}\mathsf{E}_q ~p(\mathbf{y}|\boldsymbol{\theta}_2, c_{\text{temp}})$.

% здесь мы имеем ввиду что для любого lambda, такого что мы считаем параметр неинформативными
\item \textit{Условие минимизации сложности модели:} существует  $\boldsymbol{\beta} \in U_{\beta}$ и $K_2 \in \mathbb{R}_{+}$, такие что для любых векторов гиперпараметров $\mathbf{h}_1, \mathbf{h}_2 \in U_h, Q(\mathbf{h}_1) - Q(\mathbf{h}_2) > K$,  $\mathsf{E}_q \text{log}~p(\mathbf{y}|\boldsymbol{\theta}_1, c_{\text{temp}}) = \text{log}\mathsf{E}_q ~p(\mathbf{y}|\boldsymbol{\theta}_2, c_{\text{temp}})$, количество ненулевых параметров у первой модели меньше, чем у второй.

\item \textit{Условие достижения максимума правдоподобия модели:} существует значение гиперпараметров $\boldsymbol{\beta}$, такое что оптимизация задачи эквивалента оптимизации вариационной оценки правдоподобия модели:
\vspace{-0.2cm}
\[
    \mathbf{h}^{*} = \argmax_{} p(\mathbf{y}|\mathbf{X}, \mathbf{h}', c_{\text{temp}}),\quad
    \boldsymbol{\theta}^{*} = \argmin D_\text{KL}(q| p(\mathbf{w}, \boldsymbol{\Gamma}|\mathbf{y}, \mathbf{X}, c_{\text{temp}})).
\]
\vspace{-0.6cm}
...
\item метод максимума правдоподобия: $\max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log} p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}, c_\text{temp})};$

% нет возможности оптимизации структуры
\item метод максимума апостериорной вероятности $\max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log} p(\mathbf{y}|\mathbf{X},  \boldsymbol{\theta})}\textcolor{red}{p( \boldsymbol{\theta}| \mathbf{h}, c_\text{temp})}\textcolor{OliveGreen}{p(\mathbf{h}|\boldsymbol{\beta})};$

% нет возможности оптимизации правдоподобия модели - структа не подчиняется beta
\item метод максимума вариационной оценки правдоподобия модели $\max_{\mathbf{h}} \max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log}~{p(\mathbf{y} | \mathbf{X}, \mathbf{W}, \boldsymbol{\Gamma})}} - \textcolor{red}{\text{D}_{KL}(q(\mathbf{W}, \boldsymbol{\Gamma})||p(\mathbf{w}, \boldsymbol{\Gamma}, c_\text{temp}))}\textcolor{OliveGreen}{p(\mathbf{h}|\boldsymbol{\beta})};$

\item кросс-валидация $\max_{\mathbf{h}} \textcolor{blue}{\mathsf{E}_q \text{log}p(\mathbf{y}_\text{valid}|\mathbf{X}_\text{valid}, \boldsymbol{\theta}^{*}, c_\text{temp})}\textcolor{OliveGreen}{p(\mathbf{h}|\boldsymbol{\beta})}$, $\boldsymbol{\theta}^{*} = \argmax_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log}p(\mathbf{y}_\text{train}|\mathbf{X}_\text{train}, \boldsymbol{\theta}, c_\text{temp})}\textcolor{red}{p(\boldsymbol{\theta}| \mathbf{h})}$.

% задача не является строго двууровневой, нельзя проомтимизировать ELBO
\item AIC: $\max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log} p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}, c_\text{temp})} + \textcolor{red}{|\theta_i: \theta_i \neq 0|}$;

% задача не является строго двууровневой, нельзя проомтимизировать ELBO
\item BIC: $\max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log} p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}, c_\text{temp})} + \textcolor{red}{\text{log}(m)|\theta_i: \theta_i \neq 0|}$;
...
\item перебор структуры модели:  $\max{\boldsymbol{\Gamma}'} \max_{\boldsymbol{\theta}} \textcolor{blue}{\mathsf{E}_q \text{log} p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}, c_\text{temp})}\textcolor{red}{\mathbb{I}(\boldsymbol{\Gamma} = \boldsymbol{\Gamma}') }.$
\end{enumerate}
\end{block}
\end{frame}
...
\[
 - \textcolor{OliveGreen}{\sum_{p',c \in \mathbf{P}, \mathbf{c}_{\text{comb}} c\text{D}_{KL}(\boldsymbol{\Gamma} | p') + \text{log}p(\mathbf{h}|c_1,c_2)}}, 
\]
где 
\begin{equation}
\tag{$L^{*}$}
...
\]
\end{block}
$c_\text{train}, c_\text{prior}, c_{\text{temp}}, \mathbf{c}_{\text{comb}}, \mathbf{P}$ --- метапараметры оптимизации.\\
...
\]
\item Вариационное распределение $q$ приближает апостериорное распределение $p(\mathbf{W}, \boldsymbol{\Gamma}|\mathbf{y}, \mathbf{X}, \mathbf{h}, c_\text{temp})$ наилучшим образом:
\vspace{-0.3cm}
\[
    {D}_\text{KL}(q||p(\mathbf{W}, \boldsymbol{\Gamma}|\mathbf{y}, \mathbf{X}, \mathbf{h}, c_\text{temp})) \to \min_{\boldsymbol{\theta}}.
\]
\end{enumerate}
\end{block}
% proof
...
\]
Тогда вариационные распределения $q_{\mathbf{W}}, q_{\boldsymbol{\Gamma}}$приближают апостериорные распределения $ p(\boldsymbol{\Gamma}|\mathbf{y}, \mathbf{X}, \mathbf{h}, c_\text{temp}), p(\mathbf{W}|\boldsymbol{\Gamma},\mathbf{y}, \mathbf{X}, \mathbf{h}, c_\text{temp})$ наилучшим образом:
\[
...
\]
\[=\boldsymbol{\theta} - \beta_\text{lr} \nabla L(\boldsymbol{\theta}, \mathbf{A}^{-1}, \mathbf{m})|_{\hat{\mathfrak{D}}}, 
\]
$\beta_{\text{lr}}$ --- длина шага градиентного спуска, $\boldsymbol{\theta}_0$ --- начальное значение параметров $\boldsymbol{\theta}$, $\hat{\mathfrak{D}}$ --- случайная подвыборка исходной выборки $\mathfrak{D}$.
