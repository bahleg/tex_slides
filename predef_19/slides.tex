...
\textbf{Распределение Дирихле: }$\boldsymbol{\Gamma}\sim \text{Dir}(\mathbf{s}, \lambda_\text{temp})$\\
\begin{figure}
...
\textbf{Распределение Гумбель-софтмакс: }$\boldsymbol{\Gamma}\sim \text{GS}(\mathbf{s}, \lambda_\text{temp})$\\
\begin{figure}
 \begin{minipage}[t]{.3\textwidth}
        \centering
\begin{tikzpicture}[%
...
\\$\mathbf{A}_r^{j,k} \sim \text{inv-gamma}(\lambda_1,\lambda_2)$.

\item \textbf{Структурные параметры} модели:\\$\boldsymbol{\Gamma} = \{\boldsymbol{\gamma}^{j,k}, (j,k) \in E\},$ \\$\boldsymbol{\gamma}^{j,k} \sim \text{GS}(\mathbf{s}^{j,k}, \lambda_\text{temp}).$ 
...
\[
    q \approx  \frac{p(\mathbf{y}|\mathbf{X},\mathbf{w},\boldsymbol{\Gamma})p(\mathbf{w}, \boldsymbol{\Gamma}|\mathbf{h}, \lambda_\text{temp})}{\iint\limits_{\mathbf{w}', \boldsymbol{\Gamma'}}p(\mathbf{y}|\mathbf{X},\mathbf{w}',\boldsymbol{\Gamma}')p(\mathbf{w}', \boldsymbol{\Gamma}'|\mathbf{h}, \lambda_\text{temp})d\mathbf{w}'d\boldsymbol{\Gamma}'}.
\]
\end{block} 
...
\textcolor{blue}{\mathsf{E}_q \text{log}~{p(\mathbf{y} | \mathbf{X}, \mathbf{w}, \boldsymbol{\Gamma})}} - \textcolor{red}{\text{D}_{KL}(q(\mathbf{w}, \boldsymbol{\Gamma})||p(\mathbf{w}, \boldsymbol{\Gamma}, \lambda_\text{temp}))} = \text{log}\hat{{p}}(\mathbf{y}|\mathbf{X}, \lambda_\text{temp}).
$$ 



Оценка совпадает с интегралом правдоподобия при $$D_\text{KL}(q(\mathbf{w}, \boldsymbol{\Gamma})|(p(\mathbf{w}, \boldsymbol{\Gamma}|\mathbf{y}, \mathbf{X}, \lambda_\text{temp}))=0.$$
...
\item \textit{Условие максимизации правдоподобия выборки:} существует $\boldsymbol{\lambda} \in U_{\lambda}$ и  $K_1 \in \mathbb{R}_{+}$, такие что для любых векторов гиперпараметров $\mathbf{h}_1, \mathbf{h}_2 \in  U_{h}, Q(\mathbf{h}_1) - Q(\mathbf{h}_2) > K_1:$ матожидания правдоподбия выборок: $\mathsf{E}_q \text{log}~p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}_1, \lambda_{\text{temp}})>\text{log}\mathsf{E}_q ~p(\mathbf{y}|\mathbf{X}, \boldsymbol{\theta}_2, \lambda_{\text{temp}})$.

% здесь мы имеем ввиду что для любого lambda, такого что мы считаем параметр неинформативными
\item \textit{Условие минимизации сложности модели:} существует  $\boldsymbol{\lambda} \in U_{\lambda}$ и $K_2 \in \mathbb{R}_{+}$, такие что для любых векторов гиперпараметров $\mathbf{h}_1, \mathbf{h}_2 \in U_h, Q(\mathbf{h}_1) - Q(\mathbf{h}_2) > K_2$,  $\mathsf{E}_q \text{log}~p(\mathbf{y}|\boldsymbol{\theta}_1, \lambda_{\text{temp}}) = \text{log}\mathsf{E}_q ~p(\mathbf{y}|\boldsymbol{\theta}_2, \lambda_{\text{temp}})$, количество ненулевых параметров у первой модели меньше, чем у второй.

\item \textit{Условие достижения максимума правдоподобия модели:} существует значение гиперпараметров $\boldsymbol{\lambda}$, такое что оптимизация задачи эквивалента оптимизации вариационной оценки правдоподобия модели:
\vspace{-0.2cm}
\[
...
\end{equation}
\[
= \textcolor{blue}{\lambda_\text{train}\mathsf{E}_{{q}^{*}} \text{log}~{p(\mathbf{y} | \mathbf{X}, \mathbf{w},\boldsymbol{\Gamma}, \mathbf{h}, \lambda_{\text{prior}})}}
 -\]
\vspace{-0.3cm}
\[- \textcolor{red}{\lambda_\text{prior}\text{D}_{KL}(p(\mathbf{w}, \boldsymbol{\Gamma} |\mathbf{h}, \lambda_{\text{temp}}) || q^{*}(\mathbf{w}, \boldsymbol{\Gamma}))}  -\]
\vspace{-0.3cm}
\[
 - \textcolor{OliveGreen}{\sum_{p' \in \mathbf{P}, \lambda \in \boldsymbol{\lambda}_{\text{comb}}} \lambda\text{D}_{KL}(\boldsymbol{\Gamma} | p') + \text{log}p(\mathbf{h}|\lambda_1,\lambda_2)}, 
\]
где 
\begin{equation}
\tag{$L^{*}$}
...
\end{equation}
\vspace{-0.3cm}
\[- \textcolor{red}{\lambda_\text{reg}\text{D}_{KL}(p(\mathbf{w}, \boldsymbol{\Gamma} |\mathbf{A}^{-1}, \mathbf{m}, \lambda_{\text{temp}}) || q(\mathbf{w}), q(\boldsymbol{\Gamma}))}.
\]
\end{block}
$\lambda_\text{train}, \lambda_\text{prior}, \lambda_{\text{temp}}, \boldsymbol{\lambda}_{\text{comb}}$ и параметры распределений $\mathbf{P}$ --- метапараметры оптимизации.\\
Оптимизационная задача обобщает алгоритмы оптимизации: оптимизация правдоподобия, последовательное увеличение и снижение сложности модели, полный перебор структуры.
\end{column}
...
\end{figure}
\vspace{-0.2cm}
$\boldsymbol{\lambda}_{\text{comb},1}=[0;0;0].$
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{combinations_2.png}
\end{figure}
\vspace{-0.2cm}
$\boldsymbol{\lambda}_{\text{comb},1}=[1;0;0].$
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{combinations_3.png}
\end{figure}
\vspace{-0.2cm}
$\boldsymbol{\lambda}_{\text{comb},1}=[1;1;0].$
\end{column}
\end{columns}
\end{frame}
...
Пусть $\lambda_\text{train} = \lambda_\text{prior}=\lambda_\text{reg}>1, \lambda_{\text{comb}}=0$. Тогда:
\begin{enumerate}
...
\vspace{-0.3cm}
\[
    \text{log}\hat{p}(\mathbf{y}|\mathbf{X}, \mathbf{h}, \lambda_\text{temp}) + \text{log}p(\mathbf{h}|\lambda_1,\lambda_2) \to \max_{\mathbf{h}}.
\]
...
Пусть $\boldsymbol{\theta} = [\mathbf{w}^1,\dots,\mathbf{w}^k]$ ---  начальные приближения оптимизации модели, $\lambda$ --- шаг градиентного спуска.

Тогда разность энтропий на смежных шагах оптимизации приближается следующим образом:
\small
\[
	\mathsf{E}_{q^{\tau}_{\mathbf{w}}}(-\text{log}(q^{\tau}_\mathbf{w})) -  \mathsf{E}_{q^{\tau-1}_{\mathbf{w}}}(-\text{log}(q^{\tau-1}_\mathbf{w}))  \approx  \frac{1}{k}\sum_{r=1}^k \bigl(\lambda Tr[\mathbf{H}(\mathbf{w}^r)] - \lambda^2 Tr[\mathbf{H}(\mathbf{w}^r)\mathbf{H}(\mathbf{w}^r)]  \bigr),
\]
...
\begin{block}{Теорема.}
Пусть $\lambda_\text{reg} > 0, m \gg 0, \frac{m}{\lambda_\text{reg}} \in \mathbb{N}.$ \\Тогда оптимизация функции \[L = 
\textcolor{blue}{\mathsf{E}_q \text{log}~{p(\mathbf{y} | \mathbf{X}, \mathbf{w}, \boldsymbol{\Gamma}, \mathbf{A}^{-1}, \lambda_{\text{temp}})}} -\]\[- \textcolor{red}{\lambda_\text{reg}\text{D}_{KL}(p(\mathbf{w}, \boldsymbol{\Gamma} |\mathbf{A}^{-1}, \mathbf{m}, \lambda_{\text{temp}}) || q(\mathbf{w}), q(\boldsymbol{\Gamma}))}\] эквивалентна минимизации ожидаемой дивергенции $\mathsf{E}_{\hat{\mathbf{X}}, \hat{\mathbf{y}}\sim p(\mathbf{X}, \mathbf{y})}\text{D}_{KL}(q||p(\mathbf{w}, \boldsymbol{\Gamma} | \hat{\mathbf{X}}, \hat{\mathbf{y}})),$ где $\hat{\mathbf{X}}, \hat{\mathbf{y}}$ --- случайные подвыборки мощностью $\frac{m}{\lambda_\text{reg}}$ из генеральной совопкупности.
\end{block}
...
\[
    C_p = \min_{\mathbf{h}} D_\text{KL}(q||p(\mathbf{w}, \boldsymbol{\Gamma}|\lambda_\text{temp})).
\]
\end{block}
...
%
\begin{block}{Теорема}
Пусть $\lambda_{\text{train}} = \lambda_{\text{reg}} = 1, \lambda_{\text{comb}} = 0$.
Тогда предел оптимизации
\[
\lim_{\lambda_\text{prior} \to \infty} \lim_{\eta \to \infty}   T^\eta\bigl(Q, \mathbf{h}, T^\eta(L, \boldsymbol{\theta}_0, \mathbf{h})\bigr)
\]  
...
Пусть $\lambda_{\text{train}} = 1, \lambda_{\text{comb}} = 0$.
Пусть  $\mathbf{f}_1, \mathbf{f}_2$ --- результаты градиентной оптимизации при разных значениях гиперпараметров $\lambda_{\text{prior}}^1,\lambda_{\text{prior}}^2, \lambda_{\text{prior}}^1<\lambda_{\text{prior}}^2$, полученных при начальном значении вариационных параметров $\boldsymbol{\theta}_0$ и гиперпараметров $\mathbf{h}_0$.
...
Тогда:
\[
    C_p(\mathbf{f}_1) - C_p(\mathbf{f}_2)  \geq \lambda_\text{reg}(\lambda_\text{reg} - \lambda_\text{prior}^1)\text{sup}_{\boldsymbol{\theta}, \mathbf{h} \in U}|\nabla^2_{\boldsymbol{\theta}, \mathbf{h}} D_{KL}(q|p) (\nabla^2_{\boldsymbol{\theta}} L)^{-1}   \nabla_{\boldsymbol{\theta}} D_{KL}(q|p))|.
\]
\end{block}
...
\begin{block}{Теорема}
Пусть задано априорное распределение на структуре: $$p(\boldsymbol{\gamma}_{i,j}) =  \lim_{\lambda_{\text{temp}} \to 0} \mathcal{GS}(\lambda_{\text{temp}}).$$
Пусть $\lambda_{\text{reg}} >0, \lambda_{\text{train}} >0, \lambda_{\text{prior}}>0, \lambda_{\text{comb}}=0$, $\mathbf{f}$ --- глобальный оптимум задачи оптимизации.
Тогда 
$
    C_s(\mathbf{f}) = 0.
$
...
\begin{block}{}
Пусть $$p(\boldsymbol{\gamma}_{i,j}) =  \lim_{\lambda_{\text{temp}} \to \infty} \mathcal{GS}(\lambda_{\text{temp}}).$$
...
\small
Пусть для каждого ребра $(i,j)$ семейства моделей $\mathfrak{F}$ априорное распределение $$p(\boldsymbol{\gamma}_{i,j}) =  lim_{\lambda_{\text{temp}} \to 0} \mathcal{GS}(\lambda_{\text{temp}}).$$

Рассмотрим последовательность $\mathbf{P}$, состоящую из $N = \prod_{(j,k) \in E} K_{j,k}$ моделей, полученных в ходе оптимизаций вида:
$$f_1 \in F(\lambda_{\text{reg}}, 0, 0, \varnothing, \lambda_{\text{comb}},  \lambda_{\text{temp}}),$$
$$f_2 \in F(\lambda_{\text{reg}}, 0, 0, \{q_1(\boldsymbol{\Gamma})\},  \lambda_{\text{comb}},  \lambda_{\text{temp}}),$$
$$f_3 \in F(\lambda_{\text{reg}}, 0, 0, \{q_1(\boldsymbol{\Gamma}), q_2(\boldsymbol{\Gamma})\},  \lambda_{\text{comb}},  \lambda_{\text{temp}}),$$
где $C_{\text{reg}} > 0,  \lambda_{\text{comb}}>0$.
